...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `toe`.`zone_BEFORE_INSERT` BEFORE INSERT ON `zone` FOR EACH ROW
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `toe`.`zone_BEFORE_UPDATE` BEFORE UPDATE ON `zone` FOR EACH ROW
BEGIN
	IF NEW.latitude < -90 OR NEW.latitude > 90 THEN
		CALL `bad latitude used. Must be between -90 and 90.`;
	END IF;
