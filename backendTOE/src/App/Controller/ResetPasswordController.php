...
class ResetPasswordController extends BaseController
{
	public function ResetPassword(Application $app)
	{
		...
		$qb->update('password_request')
			...
			->andWhere('issued_at = :issued_at')
			->andWhere('expired_at = :expired_at')
			...
			->setParameter(':issued_at', $data->iat)
			->setParameter(':expired_at', $data->exp);
		$qb->execute();
	 ...
	 * @param int $currentTime The time that the request was sent in
	 *
	...
	private function isValidToken($token, $currentTime)
	{
		...
		$qb->select([
			...
			->setParameter(':issued_at', $token->iat)
			->setParameter(':expired_at', $token->exp);
		$result = $qb->execute()->fetchAll();

		if (count($result) === 0 || $result[0]['expired_at'] < $currentTime)
		{
